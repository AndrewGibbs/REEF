...
%             wavelength = 2*pi/kwave; 
            ...
            if isempty(keep_incs)
                [self.alpha_in, self.ffDhat, self.FF_in, self.full_rank] = choose_submatrix(alpha_in,FF_in,self.hat,self.M,self.rank_tol);
            else
                [self.alpha_in, self.ffDhat, self.FF_in] = choose_submatrix_keeps(alpha_in,FF_in,self.hat,self.M,self.rank_tol,keeps);
            end
